...
class HyprlandProvider extends WorkspaceProvider {
  ...
  int getCurrent() {
    ProcessResult processResult =
        Process.runSync('hyprctl', ['activeworkspace', '-j']);
    if (processResult.exitCode != 0) {
      return 1;
    }
    Map<String, dynamic> current =
        Map<String, dynamic>.from(json.decode(processResult.stdout));
    return (current["id"]);
  }

  startReading() async {
    var hyprlandInstanceSignature =
  ...
  List<Map<String, dynamic>>? getInformationWorkspaces() {
    ProcessResult processResult =
        Process.runSync('hyprctl', ['workspaces', '-j']);
    if (processResult.exitCode != 0) {
      return null;
    }
    List<Map<String, dynamic>> workspaceWindows =
        List<Map<String, dynamic>>.from(json.decode(processResult.stdout));
